...
class AdaptShotConfig:
    ...
    """
    # Core execution
    # mobilenet_v3_small, not resnet18, since #36. It is the backbone whose ONNX
    # weights ship in the wheel (4.0MB against 44.8MB), which is what makes
    # `pip install adaptshot` able to predict without torch at all. Measured on
    # the smoke split it is also 4.6x faster (2.6ms against 12.0ms), and its
    # accuracy is indistinguishable at that sample size: 19/25 against 17/25,
    # 95% CIs [56.6%, 88.5%] and [48.4%, 82.8%]. resnet18 remains available and
    # is generated by scripts/export_backbones.py.
