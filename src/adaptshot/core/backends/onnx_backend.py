...
class ONNXBackend:
    ...
    def __init__(self) -> None:
        ...
        # Per-instance session cache. A functools.lru_cache on the method would
        # key on `self`, keeping every backend instance -- and up to four loaded
        # ONNX sessions with their weights -- alive for the process lifetime.
        # That matters under the <250MB target. This cache dies with the instance.
        self._sessions: dict[str, Any] = {}

    def _get_session(self, backbone_name: str) -> Any:
        """Load and cache an ONNX inference session for the given backbone."""
        cached = self._sessions.get(backbone_name)
        ...
        self._sessions[backbone_name] = session
        return session
