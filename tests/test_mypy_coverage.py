...
def mypy_result() -> subprocess.CompletedProcess[str]:
    ...
    # PEP 696 defaults, and every bare `np.ndarray` annotation became a
    # `type-arg` error there -- 157 of them -- while 3.12+ resolved numpy 2.5 and
    # saw none. The check was measuring numpy's stubs rather than our code, so it
    # was pinned to one interpreter.
    #
    # #44 parameterised every array annotation, and the count on numpy 2.2 is
    # now zero. So this runs on every matrix entry: a stub set that disagrees
    # with the code is now a finding about the code, and a failure here on one
    # Python but not another is the signal, not noise to be pinned away.
    return _run_mypy()
